...
func (h *IntelligenceHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeError(w, r, domain.ValidationError{Message: "malformed query string"})
		return
	}
	...
	if query.Has("benchmark") {
		if query.Get("benchmark") == "" {
			writeError(w, r, domain.ValidationError{Field: "benchmark", Message: "benchmark must not be empty", Err: domain.ErrInvalidTicker})
			return
		}
	...
	if query.Has("rsiWindow") {
		...
		if _, err := fmt.Sscanf(query.Get("rsiWindow"), "%dy", &years); err != nil || years <= 0 || query.Get("rsiWindow") != strconv.Itoa(years)+"y" {
			writeError(w, r, domain.ValidationError{Field: "rsiWindow", Message: "rsiWindow must use a positive number followed by y"})
			return
		}
	...
	if query.Has("includeDetails") {
		...
		case "false":
		default:
			writeError(w, r, domain.ValidationError{Field: "includeDetails", Message: "includeDetails must be true or false"})
			return
		}
	}
	if query.Has("asOf") {
		...
		if err != nil || input.AsOf.IsZero() {
			writeError(w, r, domain.ValidationError{Field: "asOf", Message: "asOf must use YYYY-MM-DD and be nonzero", Err: domain.ErrInvalidDateRange})
			return
		}
	}
	if query.Has("marketType") {
		...
		if err != nil || input.MarketType <= 0 {
			writeError(w, r, domain.ValidationError{Field: "marketType", Message: "marketType must be a positive integer", Err: domain.ErrInvalidMarketType})
			return
		}
	}
